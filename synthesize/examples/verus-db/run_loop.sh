...
[[ "$agent" == claude || "$agent" == fcc-claude || "$agent" == codex ]] || die "--agent must be claude, fcc-claude, or codex"
[[ "$codex_sandbox" == workspace-write || "$codex_sandbox" == off ]] || die "--codex-sandbox must be workspace-write or off"
...
if [[ "$agent" == codex ]]; then
  # The trailing - makes codex exec read the prompt from stdin; invoke runs it in the run directory.
  # Unlike run_codex.sh, this does not check the .codex/ wiring or project trust before launching.
  agent_command=(codex exec --skip-git-repo-check --color never)
  if [[ "$codex_sandbox" == off ]]; then
    agent_command+=(--dangerously-bypass-approvals-and-sandbox)
  else
    agent_command+=(--sandbox workspace-write)
  fi
  if ((codex_bypass_hook_trust)); then
    agent_command+=(--dangerously-bypass-hook-trust)
  fi
