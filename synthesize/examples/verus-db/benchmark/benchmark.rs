// Runtime diagnostic harness; the copied candidate and specification are unchanged.
#[path = "spec/mod.rs"]
mod db;
#[path = "impl/implementation.rs"]
mod candidate;

use candidate::VerifiedDb;
use db::Database;
...
// Same key and value encoding as the scored benchmark (evaluator/interface/benchmark.rs).
...
fn smoke() {
    ...
    store.put(key("3"), 30);
    store.put(key("9"), 99);
    assert_eq!(store.get(&key("9")), Some(&99));
    let mut range = store.scan(&key("3"), &key("9"));
    range.sort_unstable();
    assert_eq!(range, vec![(key("3"), 30), (key("9"), 99)]);
    assert!(store.scan(&key("4"), &key("8")).is_empty());
    assert_eq!(store.sort(), vec![(key("3"), 30), (key("9"), 99)]);
}
...
fn main() {
    ...
    for &key in &load {
        ...
        store.put(names[key as usize].clone(), value);
    ...
    loop {
        for _ in 0..256 {
            ...
            if reads[index] {
                let value = *black_box(store.get(&names[key])).expect("preloaded key missing");
                ...
                store.put(names[key].clone(), value);
                expected[key] = value;
            }
            operations += 1;
        }
    ...
    for i in 0..1024 {
        ...
        assert_eq!(store.get(&names[key]), Some(&expected[key]));
    }
    assert_eq!(store.get(&names[n]), None);
    println!("{{\"operations\":{operations},\"reads\":{read_count},\"writes\":{},\"seconds\":{elapsed:.9},\"ops_per_second\":{:.6},\"mops_per_second\":{:.9},\"load_seconds\":{load_seconds:.9},\"key_width\":{width},\"checksum\":{checksum},\"backing_entries\":{},\"runtime_checks_passed\":true,\"intervals\":[{}]}}",
