#!/usr/bin/env bash
# Launch the Verus database synthesis task directly through Claude Code.
set -euo pipefail
...
usage() {
  echo 'Usage: bash synthesize/examples/verus-db/run_claude.sh [--dry-run] [--resume-run DIR] [-- CLAUDE_ARGS...]'
  echo 'Starts an interactive Claude Code session with the database task already submitted.'
  echo 'Requires Claude Code to be installed and authenticated.'
...
for executable in python3 jq claude; do
...
if [ "$dry_run" -eq 1 ]; then
  ...
  printf ' %q\n' "$prompt"
  echo 'Dry run only; Claude Code is not launched.'
  exit 0
fi

# Use the same skill, role, and delivery-hook installation as skydiscover init.
bash "$installer" --agent claude "$project_dir"
echo 'Launching Claude Code. The session uses normal Claude permission prompts.'
