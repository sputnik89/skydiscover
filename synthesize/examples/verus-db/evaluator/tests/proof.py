...
HASHES = {
    "mod.rs": "2d9c2bcc77ba597c72f25e0ddb76020888a3008c362158ad4c9f9423cd533f31",
}
FORBIDDEN = {
    "assume", "admit", "assume_specification", "axiom", "external_body", "external",
    "inline_air_stmt", "exec_allows_no_decreases_clause", "assume_termination",
}
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="skysynth-verus-db-") as scratch:
        ...
        command = [binary, "database_test.rs", "--crate-type", "bin", "--crate-name",
                   "skysynth_database", "--no-cheating", "--output-json", "--expand-errors",
                   "--rlimit", "10", "--num-threads", "1", "--compile", "-o", "database"]
        env = os.environ.copy()
